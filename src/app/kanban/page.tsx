...
export default function KanbanPage() {
  ...
  const createColumn = async () => {
    if (!isAdmin) {
      setError('Apenas administradores podem criar colunas.');
      return;
    }
  ...
  const createCard = async (columnId: number) => {
    if (!isAdmin) {
      setError('Apenas administradores podem criar cards.');
      return;
    }
  ...
  const moveCard = async (cardId: number, toColumnId: number, toPosition: number) => {
    if (!isAdmin) {
      setError('Apenas administradores podem mover cards.');
      await reload();
      return;
    }
    const cardTitle = (() => {
                ...
                {isAdmin && canCreateColumn ? (
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-xl bg-blue-600 text-slate-900 dark:text-white px-4 py-2 text-sm font-semibold hover:bg-blue-700 transition"
                    >
                        ...
                        >
                          <button
                            ...
                            onClick={async (e) => {
                              e.stopPropagation();
                              if (!isAdmin) {
                                setError('Apenas administradores podem apagar cards.');
                                return;
                              }
                              try {
                                const res = await fetch('/api/kanban', {
                    ...
                    {isAdmin && (
                      <div className="mt-3 flex gap-2">
                        <button
                          ...
                          className="inline-flex items-center justify-center rounded-xl bg-slate-900 text-slate-900 dark:text-white px-3 py-2 text-sm font-semibold hover:bg-white/90 dark:bg-white dark:text-slate-900 dark:hover:bg-white/90 transition"
