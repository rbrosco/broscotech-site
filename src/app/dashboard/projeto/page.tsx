...
export default function ProjetoPage() {
  ...
  const [, setProjectsList] = useState<Project[]>([]);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/projects?userId=${uid}`, { credentials: 'include' });
        ...
        setData(resolved);
        if (resolved.project) {
          setProjectName(resolved.project.title ?? '');
          ...
          setLanguage(resolved.project.language ?? '');
          setFramework(resolved.project.framework ?? '');
          setIntegrationsField(resolved.project.integrations ?? '');
      ...
      <div className="relative md:ml-64 bg-blueGray-100 dark:bg-gray-900 w-full">
        ...
        <div className="px-4 md:px-6 mx-auto w-full pt-24 pb-10">
          <div className="rounded-3xl bg-white/90 dark:bg-gray-800/80 backdrop-blur-md border border-white/20 dark:border-gray-700/50 shadow-2xl p-5 sm:p-6">
            <h1 className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-white">Cadastre seu projeto</h1>
            ...
            {loading ? (
              ...
              <div className="mt-6 space-y-6">
                ...
                <div className="flex items-center gap-3">
                  {editMode ? (
                    <button
                      onClick={() => { void onSave(); setEditMode(false); }}
                      disabled={saving}
