...
function ProgressBar({ value }: { value?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: '#1e2a4a' }}>
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-5xl mx-auto flex flex-col gap-6">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">Visão geral de todos os clientes e seus projetos.</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={s.label} className="rounded-xl px-4 py-4" style={{ background: '#0d1224', border: '1px solid #1e2a4a' }}>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-lg text-sm text-white focus:outline-none"
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold"
                      style={{ background: '#1a2035', color: '#00b09b' }}>
                      {(g.key[0] ?? '?').toUpperCase()}
                    </div>
                    <div className="text-left">
                      <p className="text-white font-semibold text-sm">{g.key}</p>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-500">{g.projects.length} projeto{g.projects.length !== 1 ? 's' : ''}</span>
                    <FiUser size={14} className="text-gray-500" />
                  </div>
                </button>

                {/* Projects table */}
                <div className="border-t" style={{ borderColor: '#1e2a4a' }}>
                  <table className="w-full text-sm">
                    <thead>
                      <tr style={{ background: '#080c18' }}>
                        <th className="text-left px-5 py-2 text-xs text-gray-500 font-medium">Projeto</th>
                        <th className="text-left px-3 py-2 text-xs text-gray-500 font-medium hidden md:table-cell">Tipo</th>
                        <th className="text-left px-3 py-2 text-xs text-gray-500 font-medium">Status</th>
                        <th className="px-3 py-2 text-xs text-gray-500 font-medium hidden lg:table-cell">Progresso</th>
                        <th className="text-left px-3 py-2 text-xs text-gray-500 font-medium hidden lg:table-cell">Prazo</th>
                        <th className="px-3 py-2 text-xs text-gray-500 font-medium">Ações</th>
                      </tr>
                    </thead>
                    <tbody>
                      {g.projects.map((p, i) => (
                        <tr key={p.id}
                          style={{ borderTop: i > 0 ? '1px solid #1a2240' : undefined }}>
                          <td className="px-5 py-3 text-white font-medium">{p.title}</td>
                          <td className="px-3 py-3 text-gray-400 hidden md:table-cell text-xs">{p.project_type || '—'}</td>
                          <td className="px-3 py-3"><StatusBadge status={p.status} /></td>
                          <td className="px-3 py-3 hidden lg:table-cell" style={{ minWidth: 120 }}>
                            <ProgressBar value={p.progress} />
                          </td>
                          <td className="px-3 py-3 text-gray-400 text-xs hidden lg:table-cell">{p.final_date || '—'}</td>
