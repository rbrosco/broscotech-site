...
export default function IAAgentButton({ pulse = true, badge = 0 }: Props) {
  ...
  return (
    <div className="fixed right-6 bottom-6 z-60">
      ...
      {badge > 0 && (
        <span className="absolute -top-2 -right-2 inline-flex items-center justify-center w-6 h-6 rounded-full bg-red-500 text-white text-xs font-semibold shadow">{badge > 99 ? '99+' : badge}</span>
      )}
    </div>
  );
}
