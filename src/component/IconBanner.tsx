...
const IconBanner: React.FC<IconBannerProps> = ({ icons, speed = '40s' }) => {
  ...
  return (
    <div className="w-full py-10 md:py-14 overflow-hidden bg-slate-50 dark:bg-black/10">
            ...
            >
              {duplicatedIcons.map((icon, index) => (
                <div key={`row1-${index}`} className="flex-shrink-0 px-3 py-3">
                  <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 whitespace-nowrap dark:border-white/10 dark:bg-black/40">
                    <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-slate-100 dark:bg-white/10">
                      <icon.Icon className="w-5 h-5 text-slate-900 dark:text-white/90" aria-hidden="true" />
                    </span>
          ...
          <div className="max-w-5xl mx-auto rounded-3xl bg-white border border-slate-200 p-5 sm:p-6 shadow-sm dark:bg-black/10 dark:border-white/15 dark:backdrop-blur-md">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div className="hidden md:block">
                <div className="relative w-40 h-72 rounded-[2.25rem] border border-slate-200 bg-white overflow-hidden dark:border-white/20 dark:bg-black/40">
                  <div className="absolute inset-x-0 top-0 h-7 bg-slate-100 border-b border-slate-200" />
                  <div className="absolute top-2 left-1/2 -translate-x-1/2 h-1.5 w-12 rounded-full bg-slate-200" />
                  ...
                  <div className="absolute inset-0 p-4 pt-10">
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <div className="rounded-2xl bg-slate-50 border border-slate-100 p-3 dark:bg-black/40 dark:border-white/10">
                        <div className="h-6 w-6 rounded-xl bg-slate-200 dark:bg-white/15" />
                        <div className="mt-2 h-2 w-16 rounded bg-slate-200 dark:bg-white/15" />
                      </div>
                      <div className="rounded-2xl bg-slate-50 border border-slate-100 p-3 dark:bg-black/40 dark:border-white/10">
                        <div className="h-6 w-6 rounded-xl bg-slate-200 dark:bg-white/15" />
                        <div className="mt-2 h-2 w-14 rounded bg-slate-200 dark:bg-white/15" />
                      </div>
                      <div className="rounded-2xl bg-slate-50 border border-slate-100 p-3 dark:bg-black/40 dark:border-white/10">
                        <div className="h-6 w-6 rounded-xl bg-slate-200 dark:bg-white/15" />
                        <div className="mt-2 h-2 w-16 rounded bg-slate-200 dark:bg-white/15" />
                      ...
                      <div className="rounded-2xl bg-slate-50 border border-slate-100 p-3 dark:bg-black/40 dark:border-white/10">
                        <div className="h-6 w-6 rounded-xl bg-slate-200 dark:bg-white/15" />
                        <div className="mt-2 h-2 w-12 rounded bg-slate-200 dark:bg-white/15" />
                      </div>
                    </div>
