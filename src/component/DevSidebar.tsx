...
const DevSidebarContent: React.FC<DevSidebarContentProps> = ({
  isCollapsed,
  userName,
  toggleSidebar,
  setMobileOpen,
...
const DevSidebar: React.FC = () => {
  ...
  const isActive = useCallback((href: string) => (href === '/dev' ? pathname === '/dev' : (pathname?.startsWith(href) ?? false)), [pathname]);
