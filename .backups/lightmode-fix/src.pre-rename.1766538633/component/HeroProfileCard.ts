        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {['Web Apps','APIs','PostgreSQL','MongoDB','n8n'].map(tag => (
              <span key={tag} className="text-[11px] px-2 py-1 rounded-full bg-white/60 border border-black/10 dark:bg-white/5 dark:border-white/10 text-slate-700 dark:text-white/80">
                {tag}
              </span>
            ))}
          </div>
