...
const IconBanner: React.FC<IconBannerProps> = ({ icons, speed = '40s' }) => {
  ...
  return (
    <div className="w-full py-10 md:py-14 overflow-hidden bg-white/60 dark:bg-black/10">
      <div className="relative">
            ...
            >
              {duplicatedIcons.map((icon, index) => (
                <div key={`row1-${index}`} className="flex-shrink-0 px-3 py-3">
                  <div className="flex items-center gap-2 rounded-full border border-black/10 bg-white/60 px-4 py-2 whitespace-nowrap dark:border-white/10 dark:bg-gradient-to-r dark:from-white/10 dark:to-white/5">
                    <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white/60 dark:bg-white/10">
            ...
            >
              {duplicatedIcons.map((icon, index) => (
                <div key={`row2-${index}`} className="flex-shrink-0 px-3 py-3">
                  <div className="flex items-center gap-2 rounded-full border border-black/10 bg-white/60 px-4 py-2 whitespace-nowrap dark:border-white/10 dark:bg-gradient-to-r dark:from-white/5 dark:to-white/10">
                    <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white/60 dark:bg-white/10">
                      <icon.Icon className="w-5 h-5 text-slate-900/90 dark:text-white/90" aria-hidden="true" />
                    </span>
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="max-w-5xl mx-auto rounded-3xl bg-white/60 border border-black/10 backdrop-blur-md p-5 sm:p-6 dark:bg-white/10 dark:border-white/15">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div className="hidden md:block">
                <div className="relative w-40 h-72 rounded-[2.25rem] border border-black/10 bg-white/60 overflow-hidden dark:border-white/20 dark:bg-black/40">
                  <div className="absolute inset-x-0 top-0 h-7 bg-white/70 border-b border-white/10" />
              ...
              <div className="flex flex-wrap gap-2">
                <a href="/register" className="rounded-full bg-white/60 border border-black/10 px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-white/50 transition dark:bg-white/10 dark:border-white/20 dark:text-white dark:hover:bg-white/15">
                  Cadastro
                </a>
                <a href="/iaagent" className="rounded-full bg-white/60 border border-black/10 px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-white/50 transition dark:bg-white/10 dark:border-white/20 dark:text-white dark:hover:bg-white/15">
                  IA Agent
                </a>
                <a href="/dashboard" className="rounded-full bg-white/60 border border-black/10 px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-white/50 transition dark:bg-white/10 dark:border-white/20 dark:text-white dark:hover:bg-white/15">
                  Dashboard
                </a>
              </div>
            </div>
            ...
            <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {[
                ...
                <div key={title} className="rounded-2xl bg-white/60 border border-black/10 p-4 hover:bg-white/50 transition dark:bg-white/10 dark:border-white/15 dark:hover:bg-white/15">
